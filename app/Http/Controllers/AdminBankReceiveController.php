...
class AdminBankReceiveController extends \crocodicstudio\crudbooster\controllers\CBController {
	...
	public function cbInit() {
		...
		$this->table = "bank_receive";
			# END CONFIGURATION DO NOT REMOVE THIS LINE

			# START COLUMNS DO NOT REMOVE THIS LINE
		$this->col = [];
		$this->col[] = ["label"=>"No. BR","name"=>"br_number"];
...
return number_format($row->total_invoice,2,'.',',');}];
		...
		$select_code = DB::table('bank_receive')->select('br_code')->orderBy('created_at','desc')->first();
		$max_code = isset($select_code->br_code) ? $select_code->br_code : 00000;
		$data = [];
		...
		$br_number= 'BR/'.$date_code.'/'.str_pad($next_code,5,'0',STR_PAD_LEFT);


		$this->form[] = ['label'=>'BR Number','name'=>'br_number','type'=>'text','validation'=>'required','width'=>'col-sm-10', 'readonly' => false, 'value'=>$br_number ];

		$this->form[] = ['label'=>'BR Code','name'=>'br_code','type'=>'hidden','validation'=>'required','width'=>'col-sm-10', 'readonly' => false, 'value'=>$next_code ];
		...
		$journal[] = ['label'=>'Description','name'=>'master_journal_detail_description','type'=>'textarea','readonly'=>false,'required'=>false];
		$journal[] = ['label'=>'Kredit','name'=>'master_journal_detail_kredit','type'=>'money','validation'=>'required','width'=>'col-sm-10'];
		$journal[] = ['label'=>'Created At','name'=>'created_at','type'=>'date','validation'=>'required','width'=>'col-sm-10', 'readonly'=>true];
		
		$this->form[] = ['label'=>'Jurnal Detail','name'=>'master_journal_detail','type'=>'child','columns'=>$journal,'table'=>'master_journal_detail','foreign_key'=>'br_id'];
   ...
   // $(function() {
	        ...
	        // 	});
	        		...
	        		// $('#jurnaldetailmaster_journal_detail_kredit').priceFormat({
	        		// 	prefix: '',
	        		// 	clearOnEmpty: false
	        		// 	})
	    ...
	    public function hook_before_delete($id) {
	        ...
	        if(is_array($id) == 1) {
	    		...
	    		foreach ($id as $id_selected) {
	    			DB::table('master_journal_detail')->where('br_id',$id_selected)->delete();
	    		}


	    	} else {

	    		DB::table('master_journal_detail')->where('br_id',$id)->delete();
	    	}
