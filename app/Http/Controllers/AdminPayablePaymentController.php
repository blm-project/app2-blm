...
class AdminPayablePaymentController extends \crocodicstudio\crudbooster\controllers\CBController {
	...
	public function cbInit() {
		...
		$this->table = "payable_payment";
			# END CONFIGURATION DO NOT REMOVE THIS LINE

			# START COLUMNS DO NOT REMOVE THIS LINE
		$this->col = [];
		$this->col[] = ["label"=>"No. Payable Payment","name"=>"payable_number"];
		...
		$select_code = DB::table('payable_payment')->select('payable_code')->orderBy('created_at','desc')->first();
		$max_code = isset($select_code->payable_code) ? $select_code->payable_code : 00000;
		$data = [];
		...
		$payable_number= 'PYB/'.$date_code.'/'.str_pad($next_code,5,'0',STR_PAD_LEFT);




		$this->form[] = ['label'=>'No. Payable Payment','name'=>'payable_number','type'=>'text','validation'=>'required','width'=>'col-sm-10', 'readonly' => false, 'value'=>$payable_number ];

		$this->form[] = ['label'=>'No. Payable CODE','name'=>'payable_code','type'=>'hidden','validation'=>'required','width'=>'col-sm-10', 'readonly' => false, 'value'=>$next_code ];

		$this->form[] = ['label'=>'Tanggal Payment','name'=>'payable_date','type'=>'date','validation'=>'required','width'=>'col-sm-10', 'readonly' => false,'value'=>date('Y-m-d') ];



		$columns[] = ['label'=>'Account Payable',
		'name'=>'account_payable_id',
		'type'=>'datamodal',
		'datamodal_table'=>'account_payable',
		'datamodal_columns'=>'ap_number,master_vendor_name_id,description,total_invoice',
		'datamodal_select_to'=>'master_vendor_name_id:master_vendor_name_id,description:description,total_invoice:total_invoice',
		'datamodal_where'=>'status_invoice LIKE "UNPAID"',
		'datamodal_size'=>'large',
		'datamodal_columns_alias'=>'No. AP,Nama Vendor,Keterangan,Total Invoice',
		'required'=>true];

		$columns[] = ['label'=>'Nama Vendor','name'=>'master_vendor_name_id','type'=>'text','readonly'=>true,'required'=>true];
		...
		$this->form[] = ['label'=>'Payable Payment Form','name'=>'payable_payment_detail','type'=>'child','columns'=>$columns,'table'=>'payable_payment_detail','foreign_key'=>'payable_payment_id'];
	        ...
	        $('#jurnaldetailmaster_journal_detail_debit, #total_invoice, #grand_total').priceFormat({
	        	...
	        	$(function() {
	        		...
	        		setInterval(function() {
	        			...
	        			var grand_total = 0;
	        			$('#panel-form-payablepaymentform tbody .total_invoice').each(function() {
