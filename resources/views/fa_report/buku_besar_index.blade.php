  ...
  @if (Session::get('message')!='')
    ...
    <div class='panel panel-default'>
        <div class="panel-heading">
           <strong><i class='fa fa-file-excel-o'></i>&nbsp;Form Buku Besar</strong>
         </div>
    <div class='panel-body'>
      <form method='get' action='/export-bukubesar/'>
        ...
        <div class='form-group' style="width: 200px;">
          <label>COA 2</label>
          <select class="form-control select" id="coa_id" name="coa_id" autofocus>
            @foreach($result as $row)
            <option value="{{ $row->id }}" >{{ $row->master_coa_detail_name }}</option>
            @endforeach
        </select>
        </div>
