  ...
  @if (Session::get('message')!='')
    ...
    <div class='panel panel-default'>
        <div class="panel-heading">
           <strong><i class='fa fa-file-excel-o'></i>&nbsp;Form Stock Receive by Vendor</strong>
         </div>
    <div class='panel-body'>
      <form method='get' action='/export-srbyvendor/'>
        ...
        <div class='form-group' style="width: 200px;">
          <label>Vendor</label>
          <select class="form-control select" id="supplier_name" name="supplier_name" autofocus>
            <option value="" >ALL</option>
            @foreach($master_supplier as $row)
          <option value="{{ $row->id }}" >{{ $row->master_supplier_name }}</option>
            @endforeach
        </select>
        </div>
